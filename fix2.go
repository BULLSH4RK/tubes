...
func tambahdata(data *tabPlayer, n int) int {
    ...
    // Urutkan data berdasarkan id setelah menambah data (menggunakan temp)
...
// Selection sort descending berdasarkan winrate (menggunakan temp)
...
// Fungsi baru untuk mengedit data (menang/kalah dan nickname)
...
func tampilanData(n int, data tabPlayer) {
    ...
    if pilihan == 1 {
        ...
        // Urutkan data berdasarkan id (menggunakan temp)
        for i := 0; i < n-1; i++ {
            for j := i + 1; j < n; j++ {
        ...
        // sort wr dengan selection sort descending (menggunakan temp)
        for i := 0; i < n-1; i++ {
            maxIdx := i
            for j := i + 1; j < n; j++ {
                if tempData[j].wr > tempData[maxIdx].wr {
                    maxIdx = j
                }
            }
            if maxIdx != i {
                // Menggunakan temp untuk swap
                temp := tempData[i]
                tempData[i] = tempData[maxIdx]
                tempData[maxIdx] = temp
            }
        }
